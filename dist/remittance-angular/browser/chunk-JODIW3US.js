...
var ApiService = class _ApiService {
  ...
  patch(path, body) {
    return this.http.patch(this.url(path), body ?? null).pipe(catchError((err) => this.handleError(err)));
  }
  // ---------------------------------------------------------------------------
  // Auth
  // ---------------------------------------------------------------------------
  ...
  resetPassword(token, newPassword, confirmPassword) {
    return this.post("api/auth/reset-password", { token, newPassword, confirmPassword });
  }
  validateResetToken(token) {
    return this.http.get(this.url(`api/auth/validate-reset-token?token=${encodeURIComponent(token)}`)).pipe(catchError((err) => this.handleError(err)));
  }
  // ---------------------------------------------------------------------------
  // Transaction PIN
  // ---------------------------------------------------------------------------
  ...
  confirmPayout(id) {
    return this.put(`api/admin/transactions/${id}/confirm-payout`);
  }
  retryPayout(id) {
    return this.put(`api/admin/transactions/${id}/retry-payout`);
  }
  markTransactionPaid(id) {
    return this.put(`api/admin/transactions/${id}/mark-paid`);
  }
  getIntegrationLogs(id) {
    return this.get(`api/admin/transactions/${id}/integration-logs`);
  }
  ...
  createRateQuote(sourceCurrency, destinationCurrency, payoutPartnerId, country, countryIso3) {
  ...
  updateAgentCustomer(id, dto) {
    return this.put(`api/agent/customers/${id}`, dto);
  }
  patchCustomerFields(id, fields) {
    return this.patch(`api/agent/customers/${id}/fields`, fields);
  }
  // Agent: Receivers
  ...
  updateAgentReceiver(id, dto) {
    return this.put(`api/agent/receivers/${id}`, dto);
  }
  patchReceiverFields(id, fields) {
    return this.patch(`api/agent/receivers/${id}/fields`, fields);
  }
  getAgentFieldMappings(payoutAgentId, paymentMethod, country) {
    const parts = [];
    if (paymentMethod)
      parts.push(`paymentMethod=${encodeURIComponent(paymentMethod)}`);
    if (country)
      parts.push(`country=${encodeURIComponent(country)}`);
    const params = parts.length ? `?${parts.join("&")}` : "";
    return this.get(`api/agent/field-mappings/${payoutAgentId}${params}`);
  }
  ...
  getAgentBanksForPayout(agentId, country, paymentMethodId, additionalFields) {
    const params = [];
    if (country)
    ...
    if (additionalFields)
      params.push(`additionalFields=${encodeURIComponent(additionalFields)}`);
    const url = params.length ? `api/agent/banks/agent/${agentId}?${params.join("&")}` : `api/agent/banks/agent/${agentId}`;
    return this.get(url);
  }
  getMoneyGramPayoutFields(serviceOptionCode, sendingCountry, payoutCountry, amount, serviceOptionRoutingCode) {
    const params = [`serviceOptionCode=${encodeURIComponent(serviceOptionCode)}`];
    if (sendingCountry)
      params.push(`sendingCountry=${encodeURIComponent(sendingCountry)}`);
    if (payoutCountry)
      params.push(`payoutCountry=${encodeURIComponent(payoutCountry)}`);
    if (amount != null)
      params.push(`amount=${amount}`);
    if (serviceOptionRoutingCode)
      params.push(`serviceOptionRoutingCode=${encodeURIComponent(serviceOptionRoutingCode)}`);
    return this.get(`api/utility/moneygram/payout-fields?${params.join("&")}`);
  }
  // Agent: Cash pickup locations for a payout agent (banks with cash payment method)
  getAgentCashLocations(agentId) {
    return this.get(`api/agent/banks/cash-locations/${agentId}`);
  }
  // Agent: Wallet/mobile money locations for a payout agent (filtered by wallet payment method)
  getAgentWalletLocations(agentId) {
    return this.get(`api/agent/locations/wallet-locations/${agentId}`);
  }
  getReceiverPaymentDetails(receiverId, methodType, country, payoutPartnerId) {
    const qs = new URLSearchParams();
    if (country)
      qs.set("country", country);
    if (payoutPartnerId)
      qs.set("payoutPartnerId", payoutPartnerId.toString());
    const q = qs.toString() ? `?${qs.toString()}` : "";
    return this.get(`api/agent/receivers/${receiverId}/payment-details/${methodType}${q}`);
  }
  getMoneyGramServiceOptions(country) {
    return this.get(`api/agent/moneygram/service-options?country=${encodeURIComponent(country)}`);
  }
  ...
  getAgentBanksPaged(agentId, params) {
    ...
    if (params.paymentMethodId)
      p.set("paymentMethodId", String(params.paymentMethodId));
    if (params.isActive != null)
      p.set("isActive", String(params.isActive));
    const qs = p.toString() ? `?${p.toString()}` : "";
    return this.get(`api/admin/agent-banks/agent/${agentId}/paged${qs}`);
  }
  createAgentBank(dto) {
  ...
  getUserIpWhitelist(userId) {
    return this.get(`api/admin/user-management/users/${userId}/ip-whitelist`);
  }
  addUserIpWhitelist(userId, ipAddress) {
    return this.post(`api/admin/user-management/users/${userId}/ip-whitelist`, { ipAddress });
  }
  removeUserIpWhitelist(userId, ipAddress) {
    return this.delete(`api/admin/user-management/users/${userId}/ip-whitelist/${encodeURIComponent(ipAddress)}`);
  }
  getPrivileges() {
  ...
  getAgentTransactionPayoutLogs(id) {
    return this.get(`api/agent/transactions/${id}/payout-logs`);
  }
  // ---------------------------------------------------------------------------
  ...
  updateMyProfile(dto) {
    return this.put("api/auth/profile", dto);
  }
  changePassword(dto) {
  ...
  // ---------------------------------------------------------------------------
  getMyStatement(params) {
    const q = new URLSearchParams();
    if (params.startDate)
      q.set("startDate", params.startDate);
    if (params.endDate)
      q.set("endDate", params.endDate);
    return this.get(`api/agent/reports/statement?${q.toString()}`);
  }
  // Ingested Entries (OFAC / UN / EU / UK auto-ingested)
  getIngestedEntriesPaged(params) {
    const q = new URLSearchParams();
    ...
    return this.get(`api/admin/sanctions/ingested/paged?${q.toString()}`);
  }
  // Screening Results
  getScreeningResultsPaged(request, statusFilter) {
    return this.get(this.buildPagedQuery("api/admin/sanctions/screenings/paged", request, statusFilter ? { statusFilter } : void 0));
  }
...
//# sourceMappingURL=chunk-JODIW3US.js.map
